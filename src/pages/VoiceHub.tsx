...
async function streamChat({ messages, language, onDelta, onDone }: { messages: Msg[]; language: string; onDelta: (t: string) => void; onDone: () => void }) {
...
const VoiceHub = () => {
  ...
  const friendlyRecognitionError = (code: string): { title: string; description: string } | null => {
    switch (code) {
      case 'no-speech':
      case 'aborted':
        return null; // silent
      ...
      case 'service-not-allowed':
        return {
          ...
          description: 'Please allow microphone access in your browser (lock icon → Site settings → Microphone → Allow), then try again.',
        };
      case 'audio-capture':
        return { title: 'No microphone found', description: 'Connect a microphone and try again.' };
      case 'network':
        return { title: 'Network error', description: 'Speech service is unreachable. Check your internet connection.' };
      case 'language-not-supported':
        return { title: 'Language not supported', description: 'This language is not supported for voice input on your browser. Try English or type your question.' };
      default:
        return { title: 'Speech error', description: `Could not start voice input (${code}). You can type your question instead.` };
    }
  };

  const ensureMicPermission = async (): Promise<boolean> => {
    // Secure context check (SpeechRecognition needs HTTPS or localhost)
    ...
    try {
      if (navigator.permissions && (navigator.permissions as any).query) {
        ...
        if (status.state === 'denied') {
          showMicError(
            'Microphone blocked',
            "Microphone access is blocked. Click the lock icon in your browser's address bar → Site settings → Microphone → Allow, then reload the page."
          );
          return false;
        }
      }
    } catch {
      // Some browsers throw for 'microphone' — safe to ignore and fall through to getUserMedia.
    }
    // Trigger the native prompt from the user gesture; release tracks immediately.
  ...
  const startListening = async () => {
    ...
    recognition.onerror = (e: any) => {
      setIsListening(false);
      const msg = friendlyRecognitionError(String(e?.error || 'unknown'));
      if (msg) showMicError(msg.title, msg.description);
    };
    setTranscript('');
    try {
      recognition.start();
            ...
            {messages.map((m, i) => (
              ...
              >
                ...
                <div className="flex flex-col gap-1 max-w-[85%] sm:max-w-[80%]">
                  <div className={`rounded-2xl text-xs sm:text-sm leading-relaxed ${
                    m.role === 'user'
                  ...
                  {m.role === 'assistant' && !isStreaming && (
                    ...
                    >
                      <Volume2 className={`h-3 w-3 ${speakingIndex === i ? 'animate-pulse' : ''}`} /> {t('voice.listen')}
                    </motion.button>
                  )}
                </div>
                {m.role === 'user' && (
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            {Array.from({ length: 24 }).map((_, i) => (
              <motion.div key={i} className="w-0.5 rounded-full bg-primary"
                ...
                transition={{ duration: 0.5 + Math.random() * 0.3, repeat: Infinity, ease: 'easeInOut', delay: i * 0.03 }}
              />
            ))}
          </motion.div>
        )}
